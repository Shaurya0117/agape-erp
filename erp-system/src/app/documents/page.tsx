  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className="text-slate-600 mt-2">Manage official protocols and accounting vouchers (receipts).</p>
      </div>
          ...
          <div className="bg-white rounded-none shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="p-4">
              <form action={createVoucher as any} className="space-y-3 mb-6 bg-slate-50 p-4 rounded-none border border-slate-100">
                ...
                <button type="submit" className="w-full bg-slate-900 text-white text-sm py-2 rounded-none hover:bg-slate-700 transition font-medium">Log Voucher</button>
              </form>

              <div className="space-y-3">
                {vouchers.length === 0 ? (
                  <p className="text-sm text-slate-600 text-center py-4">No vouchers recorded.</p>
                ) : (
                  vouchers.map(v => (
                    <div key={v.id} className="p-3 border border-slate-100 rounded-none hover:bg-slate-50 transition">
                      ...
                      {v.article ? (
                        ...
                        <p className="text-xs text-slate-600 mt-2">Unattached</p>
                      )}
                    </div>
                  ))
                )}
          ...
          <div className="bg-white rounded-none shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="p-4">
              ...
              <div className="divide-y divide-slate-100 border border-slate-100 rounded-none">
                {protocols.length === 0 ? (
                  <p className="text-sm text-slate-600 text-center py-4">No protocols found.</p>
                ) : (
                  protocols.map(p => (
                    <div key={p.id} className="p-3 flex justify-between items-center group hover:bg-slate-50 transition">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-600">{new Date(p.date).toLocaleDateString()}</p>
                        </div>
                      </div>
                    </div>
                  ))
